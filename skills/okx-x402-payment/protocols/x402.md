...
> **Entry point**: this file is loaded by the dispatcher (`../SKILL.md`) when the 402 response carries a `PAYMENT-REQUIRED` header (v2) or an `x402Version` field in the body (v1). Pre-flight checks, skill routing, and chain support are owned by the dispatcher — start here at "Background".
...
Save `decoded` for later — you will need `decoded.x402Version` and `decoded.resource` (v2) when assembling the payment header in Step 4.
...
### Step 2: Check Wallet Status (only after user explicitly confirms payment)
...
- **Logged in** → proceed to Step 3 (Sign via TEE).
...
Run `onchainos wallet login` (AK login, no email) or `onchainos wallet login <email>` (OTP login), then proceed to Step 3.
...
## Cross-Skill Workflows (x402-specific)
...
**Step 2–3** — check wallet + sign (CLI selects aggr_deferred automatically):
...
**Step 4** — assemble v2 header (aggr_deferred, merge sessionCert into existing extra):
...
- A private key is available (via `EVM_PRIVATE_KEY` env var or `~/.onchainos/.env`) — Step 2 already verified this by reading the file
...
Same as the main flow Step 1 — decode from `PAYMENT-REQUIRED` header (v2) or response body (v1):
...
Same as the main flow Step 4 — build the `paymentPayload` per version:
...
- **Not logged in**: Ask user to choose between wallet login or local private key signing (see Step 2). If local: read `~/.onchainos/.env` to check for a key — the CLI derives the address automatically. If wallet: proceed with `onchainos wallet login`
